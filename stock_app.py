...
def load_data(sid):
    try:
        # 抓取資料，排除新版 yfinance 的 MultiIndex 問題
...
def run_trend_prediction(df, periods=7):
    # 使用多項式回歸擬合趨勢 (絕對穩定的預測邏輯)
    y = df['Close'].fillna(method='ffill').values
    x = np.arange(len(y))
    # 使用二次多項式擬合，能抓到簡單的轉折趨勢
    z = np.polyfit(x, y, 2)
    p = np.poly1d(z)
...
if df is None:
    st.error("❌ 無法獲取數據。請檢查代號格式是否正確（例：2330.TW）。")
else:
    ...
    with tab2:
        ...
        if st.button("啟動趨勢運算"):
            with st.spinner("計算中..."):
                ...
                ax.plot(np.arange(30, 37), forecast, label="未來 7 天預測趨勢", color="#ff7f0e", linestyle="--", marker='o')
